...
# 用法: onboard.sh <目标ssh(如 ubuntu@198.51.100.88)> <模型名> [学习窗秒数=1800]
set -euo pipefail
...
$SCP ~/defense-lab/range/scripts/vm_benign_driver.sh "$TARGET:/tmp/driver.sh" >/dev/null 2>&1
$HOST_PART "chmod +x /tmp/tracee-bin /tmp/driver.sh"

for try in $(seq 1 $MAX_TRY); do
  echo "-- 第 $try 轮学习窗 ${WIN}s --" | tee -a "$REPORT"
  $HOST_PART "echo <vm-password> | sudo -S bash -c 'nohup /tmp/tracee-bin --events sched_process_exec,security_socket_connect --output json > /tmp/cap.jsonl 2>/dev/null & nohup /tmp/driver.sh $WIN > /dev/null 2>&1 &'"
  sleep "$WIN"
  $HOST_PART "echo <vm-password> | sudo -S pkill tracee-bin; echo <vm-password> | sudo -S pkill -f driver.sh" 2>/dev/null || true
  $HOST_PART "echo <vm-password> | sudo -S cat /tmp/cap.jsonl" >> "$RAW"
  N=$(wc -l < "$RAW")

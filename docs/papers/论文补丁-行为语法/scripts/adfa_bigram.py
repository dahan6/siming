...
ARCH = sys.argv[sys.argv.index("--arch") + 1] if "--arch" in sys.argv else "tinygpt"
BATCH, LR = 256, 3e-4
...
class DeepLogLSTM(nn.Module):
    """DeepLog 风格 LSTM 参照（同 token 流，隔离架构差异）"""

    def __init__(self, vocab_size, d_model=128, n_layer=2):
        super().__init__()
        self.tok = nn.Embedding(vocab_size, d_model)
        self.lstm = nn.LSTM(d_model, d_model, num_layers=n_layer, batch_first=True)
...
def main():
    ...
    # SKIP>0 时为跳步 bigram：取 (t[i], t[i+1+SKIP]) 首尾对（ADFA 原文核心特征）
    def pairs(t):
        if SKIP == 0:
            return zip(t, t[1:])
        return zip(t, t[1 + SKIP:])
    cnt = Counter()
    for t in train:
        cnt.update(pairs(t))
    bstoi = {b: i + 1 for i, (b, c) in enumerate(cnt.most_common()) if c >= 5}
    print(f"bigram 词表 {len(bstoi)}（min_freq=5，skip={SKIP}，训练集共 {sum(cnt.values())} 对）")

    def encode(t):
        return [bstoi.get(b, 0) for b in pairs(t)]
    ...
    model = TinyGPT(vocab).to(DEVICE) if ARCH == "tinygpt" else DeepLogLSTM(vocab).to(DEVICE)
    print(f"token 总数 {len(ids)} 架构 {ARCH} 参数量 {sum(p.numel() for p in model.parameters())/1e6:.2f}M")
    ...
    for cls, traces in sorted(attacks.items()):
        ...
        per_class[cls] = {"n": len(traces), "q_det": d1, "fused_det": d2,
                          "unk_rate": float(u.mean())}
    ...
    if OUT:
        ...
        with open(OUT, "w") as f:
            _json.dump({"seed": SEED, "skip": SKIP, "epochs": EPOCHS, "arch": ARCH,
                        "vocab": vocab, "thr_q": float(thr_q), "thr_u": float(thr_u),
                        "fpr_q": fpr_q, "fpr_f": fpr_f,
                        "macro_q": float(np.mean(dq)), "macro_f": float(np.mean(df)),
                        "per_class": per_class}, f, ensure_ascii=False, indent=2)
        print(f"结果 -> {OUT}")

...
小型因果 Transformer 学习正常网络流 token 流的条件分布 p(x_t | x_<t)。
异常分 = 交叉熵（惊讶度）。直接复用司命 TinyGPT 架构。
...
from fw_paths import MODEL_DIR
...
class TinyGPT(nn.Module):
    """因果 Transformer：学习 token 序列的条件分布"""
...
def main():
    path = sys.argv[1]
    out_dir = sys.argv[2] if len(sys.argv) > 2 else MODEL_DIR
